      ...
      fse.outputFile(path.join(obj.source, 'metadata.xml'), metadata(obj), function(err) {
        if (err) {
          ...
          if (obj.package.zip) {
            ...
            var fsValidPackageName = cleanAndTrim(obj.package.name);
            fse.ensureDirSync(obj.package.outputFolder);
            var zipOutput = path.join(obj.package.outputFolder, fsValidPackageName+"_v"+obj.package.version+"_"+obj.package.date+".zip");
